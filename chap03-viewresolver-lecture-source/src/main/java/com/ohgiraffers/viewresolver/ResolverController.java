...
public class ResolverController {
    ...
    public String stringReturning(Model model) {
        ...
        /*
        * 문자열로 뷰 이름을 반환한다는 것은 반환 후 ThymeleafViewResolver에게 resources/templates/를 prefix로 지정하고 .html을 suffix로 하여 resources/templates/result.html
        * 파일로 응답뷰로 설정하라는 의미
        * */
    }
    ...
    public String stringRedirectFlashAttribute(RedirectAttributes rttr) {
        /*
        * 리다이렉트시 flash 영역에 담아서 redirect할 수 있다.
        * 자동으로 모델에 추가되기 때문에 requestScope에서 꺼내서 사용할 수 있다.
        *
        * 주의!
        * 세션에 임시로 값을 담고 소멸하는 방식이기 때문에 session에 동일한 키 값이 존재하면 안된다.
        * */
